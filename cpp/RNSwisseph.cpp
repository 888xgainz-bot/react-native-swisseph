#include "RNSwisseph.h"
  ...
  std::tuple<int, int, int, int, int, double> swe_utc_time_zone(
    ...
    double osec;
    ::swe_utc_time_zone(iyear, imonth, iday, ihour, imin, isec, timezone, &oyear, &omonth, &oday,
  &ohour, &omin, &osec);
    return std::make_tuple(oyear, omonth, oday, ohour, omin, osec);
  }

  std::map<std::string, double> swe_utc_to_jd(int year, int month, int day, int hour, int min,
  double sec, int gregflag) {
    double dret[2];
    char serr[AS_MAXCH];
  ...
  std::map<std::string, double> swe_calc_ut(double tjd_ut, int ipl, int iflag) {
    ...
    return {{"longitude", xx[0]}, {"latitude", xx[1]}, {"distance", xx[2]}, {"longitudeSpeed",
  xx[3]}, {"latitudeSpeed", xx[4]}, {"distanceSpeed", xx[5]}};
  }

  std::map<std::string, double> swe_cotrans(double longitude, double latitude, double distance,
  double eps) {
  ...
  std::map<std::string, std::vector<double>> swe_houses(double tjd_ut, int iflag, double geolat,
  double geolon,const std::string &hsys) {
    ...
    return {{"cusp", std::vector<double>(cusps, cusps + 13)}, {"ascmc",
  std::vector<double>(ascmc, ascmc + 10)}};
  }

  std::map<std::string, std::vector<double>> swe_houses_armc(double armc, double geolat, double
  eps,const std::string &hsys) {
    double cusps[13], ascmc[10];
    char hsys_char = hsys[0];
  ...
  std::map<std::string, double> swe_house_pos(double armc, double geolat, double eps, const
  std::string &hsys) {
    double cusps[13];
    char serr[AS_MAXCH];
    char hsys_char = hsys[0];
  ...
  std::map<std::string, std::vector<double>> swe_nod_aps_ut(double tjd_ut, int ipl, int iflag,int
  method) {
    ...
    return {{"ascending", std::vector<double>(xnasc, xnasc + 6)}, {"descending",
  std::vector<double>(xndsc, xndsc + 6)},{"perihelion", std::vector<double>(xperi, xperi +
  6)},{"aphelion", std::vector<double>(xaphe, xaphe + 6)}};
  }
  ...
  std::vector<double> swe_heliacal_pheno_ut(double tjd_ut, std::vector<double> dgeo,
  std::vector<double> datm, std::vector<double> dobs, const std::string& object_name, int
  event_type, int helflag) {
    double darr[50];
    char serr[AS_MAXCH];
    char name[AS_MAXCH];
    ...
    int32 result = ::swe_heliacal_pheno_ut(tjd_ut, dgeo.data(), datm.data(), dobs.data(), name,
  event_type, helflag, darr, serr);
  ...
  std::vector<double> swe_heliacal_ut(double tjd_ut, std::vector<double> dgeo,
  std::vector<double> datm, std::vector<double> dobs, const std::string& object_name, int
  event_type, int helflag) {
    double dret[50];
    char serr[AS_MAXCH];
    ...
    std::strncpy(name, object_name.c_str(), AS_MAXCH - 1);
    name[AS_MAXCH - 1] = '\0';
    int32 result = ::swe_heliacal_ut(tjd_ut, dgeo.data(), datm.data(), dobs.data(), name,
  ...
  std::vector<double> swe_vis_limit_mag(double tjd_ut, std::vector<double> dgeo,
  std::vector<double> datm, std::vector<double> dobs, const std::string& object_name, int
  helflag) {
    double dret[50];
    char serr[AS_MAXCH];
    ...
    std::strncpy(name, object_name.c_str(), AS_MAXCH - 1);
    name[AS_MAXCH - 1] = '\0';
    int32 result = ::swe_vis_limit_mag(tjd_ut, dgeo.data(), datm.data(), dobs.data(), name,
  helflag, dret, serr);
  ...
  std::map<std::string, double> swe_rise_trans(double tjd_ut, int ipl, int epheflag, int rsmi,
      double geolon, double geolat, double altitude, double atpress, double attemp) {
    double geopos[3] = {geolon, geolat, altitude};
    double tret[1];
    char serr[AS_MAXCH];
    int32 result = ::swe_rise_trans(tjd_ut, ipl, nullptr, epheflag, rsmi, geopos, atpress,
  attemp, tret, serr);
    // -1 = ERR (throw); -2 = circumpolar / no rise-set (pass through -> JS maps to null).
    if (result == -1) {
      throw std::runtime_error(serr);
    }
    return {{"tret", tret[0]}, {"result", static_cast<double>(result)}};
  }
  }
